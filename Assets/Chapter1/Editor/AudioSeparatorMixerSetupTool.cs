...
namespace DormitoryMystery.Chapter1.Editor
{
    public static class AudioSeparatorMixerSetupTool
    {
        ...
        public static MixerSetupReport RunSetup(bool showDialog)
        {
            ...
            NormalizeDuplicatePrefabOverridesIfNeeded(device, report);
            ...
            if (NormalizeDuplicatePrefabOverridesIfNeeded(device, report))
            {
                mixer = ConfigureDeviceMixer(device, missionManager, lanController, stemClips, report);
            }
        ...
        private static void ValidateScene(Scene scene, MixerSetupReport report)
        {
            ...
            if (mixer != null)
            {
                ...
                report.Add(faders.Length == LanAudioRecordingCatalog.StemCount, $"Mixer has exactly 6 faders ({faders.Length}).");
                report.Add(sources.Length == LanAudioRecordingCatalog.StemCount, $"Mixer has exactly 6 AudioSources ({sources.Length}).");
                report.Add(buttons.Length == 3, $"Mixer has exactly 3 Play/Reset/Save buttons ({buttons.Length}).");
        ...
        private static bool NormalizeDuplicatePrefabOverridesIfNeeded(GameObject device, MixerSetupReport report)
        {
            int faderCount = device.GetComponentsInChildren<AudioStemFader>(true).Length;
            int sourceCount = device.GetComponentsInChildren<AudioSource>(true).Length;
            int buttonCount = device.GetComponentsInChildren<AudioSeparatorMixerButton>(true).Length;
            bool hasDuplicates = faderCount > LanAudioRecordingCatalog.StemCount ||
                sourceCount > LanAudioRecordingCatalog.StemCount ||
                buttonCount > 3;
            if (!hasDuplicates)
            {
                return false;
            ...
            GameObject instanceRoot = PrefabUtility.GetOutermostPrefabInstanceRoot(device);
            if (instanceRoot != device)
            {
                report.Warning(
                    $"Detected duplicate mixer controls ({faderCount} faders, {sourceCount} sources, {buttonCount} buttons), " +
                    "but AudioSeparator_Device is not a prefab instance root, so setup preserved it for manual review.");
                return false;
            }

            Transform transform = device.transform;
            Transform parent = transform.parent;
            Vector3 localPosition = transform.localPosition;
            Quaternion localRotation = transform.localRotation;
            Vector3 localScale = transform.localScale;
            int siblingIndex = transform.GetSiblingIndex();
            string objectName = device.name;
            int layer = device.layer;
            bool activeSelf = device.activeSelf;

            PrefabUtility.RevertPrefabInstance(device, InteractionMode.AutomatedAction);
            ...
            device.layer = layer;
            device.SetActive(activeSelf);
            PrefabUtility.RecordPrefabInstancePropertyModifications(transform);
            PrefabUtility.RecordPrefabInstancePropertyModifications(device);

            report.Pass(
                $"Removed duplicate scene-only mixer overrides ({faderCount} faders, {sourceCount} sources, {buttonCount} buttons) " +
                "and restored the prefab instance placement.");
            return true;
        }
