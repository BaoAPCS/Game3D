...
namespace DormitoryMystery.Chapter1.Editor
{
    public static class Mission01AudioSeparatorSetupTool
    {
        ...
        private const string CalendarWallName = "Wall_Room_Nam_South_Left";
        ...
        private static void EnsureCalendarInteractable(Scene scene, int interactableLayer, SetupResult report)
        {
            ...
            if (calendar == null)
            {
                GameObject calendarPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(CalendarPrefabPath);
                GameObject calendarWall = FindSceneObject(scene, CalendarWallName);
                if (calendarPrefab == null || calendarWall == null)
                {
                    report.Warning(
                        $"{CalendarObjectName} not found and could not be placed automatically. " +
                        $"Prefab: {(calendarPrefab != null ? "found" : "missing")}; " +
                        $"wall '{CalendarWallName}': {(calendarWall != null ? "found" : "missing")}.");
                    return;
                }

                calendar = PrefabUtility.InstantiatePrefab(calendarPrefab, scene) as GameObject;
                if (calendar == null)
                {
                    report.Warning($"Could not instantiate calendar prefab: {CalendarPrefabPath}.");
                    return;
                }

                calendar.name = CalendarObjectName;
                calendar.transform.SetParent(calendarWall.transform, false);
                calendar.transform.localPosition = new Vector3(0f, 0f, 0.58f);
                calendar.transform.localRotation = Quaternion.identity;
                calendar.transform.localScale = new Vector3(0.5882353f, 0.33333334f, 4f);
                report.Pass($"Placed missing {CalendarObjectName} on {CalendarWallName} using the main-scene transform.");
            }
