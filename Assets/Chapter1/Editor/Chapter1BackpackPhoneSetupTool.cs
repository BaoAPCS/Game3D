...
namespace DormitoryMystery.Chapter1.Editor
{
    public static class Chapter1BackpackPhoneSetupTool
    {
        ...
        private static GameObject EnsurePhonePanelPrefab(UiAssets assets, Report report)
        {
            ...
            if (existing != null)
            {
                GameObject contents = PrefabUtility.LoadPrefabContents(PhonePanelPrefabPath);
                try
                {
                    PhoneUIController existingController = contents.GetComponent<PhoneUIController>() ?? contents.AddComponent<PhoneUIController>();
                    EnsurePhonePanelLayout(contents.transform, assets);
                    ConfigurePhoneControllerReferences(existingController, null, assets);
                    PrefabUtility.SaveAsPrefabAsset(contents, PhonePanelPrefabPath);
                }
                finally
                {
                ...
                report.Pass($"PhonePanel prefab exists and layout is up to date: {PhonePanelPrefabPath}.");
                return AssetDatabase.LoadAssetAtPath<GameObject>(PhonePanelPrefabPath);
            }
        ...
        private static void ConfigurePhoneControllerReferences(PhoneUIController controller, PlayerInputLock inputLock, UiAssets assets)
        {
            ...
            SetObject(serialized, "canvasGroup", controller.GetComponent<CanvasGroup>());
            SetObject(serialized, "inputLock", inputLock);
            SetObject(serialized, "homeScreen", FindChild(controller.transform, "HomeScreen"));
            SetObject(serialized, "appContent", FindChild(controller.transform, "AppContent"));
            ...
            SetObject(serialized, "closeButton", FindChildComponent<Button>(controller.transform, "CloseButton"));
            SetObject(serialized, "messagesContent", FindChild(controller.transform, "MessagesContent"));
            SetObject(serialized, "recordingsContent", FindChild(controller.transform, "RecordingsContent"));
            SetObject(serialized, "cluesContent", FindChild(controller.transform, "CluesContent"));
        ...
        private static void ValidatePrefabs(Report report)
        {
            ...
            report.Add(phone != null && HasPhoneAppButtons(phone.transform), "PhonePanel has Messenger, Ghi am, Camera, and Google app buttons.");
        }
        ...
        private static void ValidateScene(Report report)
        {
            ...
            report.Add(phoneUi != null && HasPhoneAppButtons(phoneUi.transform), "Scene PhonePanel has Messenger, Ghi am, Camera, and Google app buttons.");
